...
describe("Usuários", () => {
    ...
    beforeAll(async () => {
        ...
        const loginRes = await request(BASE_URL)
            .post('/login')
            .send({ email: 'admin@gmail.com', senha: senhaAdmin });
    ...
    it("Não deve cadastrar usuário sem nome, email ou senha (400)", async () => {
        await request(BASE_URL).post("/usuarios").send({ email: faker.internet.email(), senha: 'Senha1234!' }).set("Authorization", `Bearer ${token}`).expect(400);
        await request(BASE_URL).post("/usuarios").send({ nome: faker.name.firstName(), senha: 'Senha1234!' }).set("Authorization", `Bearer ${token}`).expect(400);
        await request(BASE_URL).post("/usuarios").send({ nome: faker.name.firstName(), email: faker.internet.email() }).set("Authorization", `Bearer ${token}`).expect(400);
    });

    it("Não deve cadastrar usuário com email duplicado (409 ou 400)", async () => {
        const email = faker.internet.email();
        await request(BASE_URL).post("/usuarios").send({ nome: faker.name.firstName(), email, senha: 'Senha1234!' }).set("Authorization", `Bearer ${token}`).expect(201);
        await request(BASE_URL).post("/usuarios").send({ nome: faker.name.firstName(), email, senha: 'Senha1234!' }).set("Authorization", `Bearer ${token}`).expect(res => {
    ...
    it("Não deve atualizar email ou senha via update (PUT)", async () => {
        ...
        const res1 = await request(BASE_URL).post("/usuarios").send({ nome: faker.name.firstName(), email, senha }).set("Authorization", `Bearer ${token}`).expect(201);
    ...
    it("Deve deletar usuário (DELETE)", async () => {
        const email = faker.internet.email();
        const senha = 'Senha1234!';
        const res1 = await request(BASE_URL).post("/usuarios").send({ nome: faker.name.firstName(), email, senha, ativo: true }).set("Authorization", `Bearer ${token}`).expect(201);
