...
describe('UsuarioRepository', () => {
  ...
  beforeAll(async () => {
    ...
    // Registrar o model de Curso para o populate funcionar
  ...
  describe('Cadastro de usuário', () => {
    it('deve cadastrar usuário válido com senha criptografada', async () => {
      ...
      const usuario = await usuarioRepository.criar({ ...usuarioBase, senha: senhaHash });
    ...
    it('não deve permitir e-mail duplicado', async () => {
      const senhaHash = await bcrypt.hash(usuarioBase.senha, 10);
      await usuarioRepository.criar({ ...usuarioBase, senha: senhaHash });
      await expect(usuarioRepository.criar({ ...usuarioBase, senha: senhaHash })).rejects.toThrow();
    });
    ...
    it('deve permitir definir ehAdmin e ativo como true', async () => {
      const senhaHash = await bcrypt.hash(usuarioBase.senha, 10);
      const usuario = await usuarioRepository.criar({ ...usuarioBase, senha: senhaHash, ehAdmin: true, ativo: true });
  ...
  describe('Leitura e listagem', () => {
    beforeEach(async () => {
      await UsuarioModel.create([
        { nome: 'João', email: 'joao@teste.com', senha: await bcrypt.hash('Senha@123', 10), ativo: true, ehAdmin: false },
        { nome: 'Maria', email: 'maria@teste.com', senha: await bcrypt.hash('Senha@123', 10), ativo: false, ehAdmin: false },
        { nome: 'Pedro', email: 'pedro@teste.com', senha: await bcrypt.hash('Senha@123', 10), ativo: true, ehAdmin: true },
      ]);
    });
    ...
    it('deve buscar usuário por id', async () => {
      const usuario = await UsuarioModel.findOne({ email: 'joao@teste.com' });
      const req = { params: { id: usuario._id.toString() }, query: {} };
      const resultado = await usuarioRepository.listar(req);
  ...
  describe('Atualização de usuário', () => {
    ...
    it('deve atualizar nome do usuário', async () => {
      const atualizado = await usuarioRepository.atualizar(usuarioExistente._id, { nome: 'Novo Nome' });
      expect(atualizado.nome).toBe('Novo Nome');
    });
    it('não deve permitir atualizar email', async () => {
      const atualizado = await usuarioRepository.atualizar(usuarioExistente._id, { email: 'novo@email.com' });
      expect(atualizado.email).toBe('atualizar@teste.com');
    });
    it('não deve permitir atualizar senha diretamente', async () => {
      const atualizado = await usuarioRepository.atualizar(usuarioExistente._id, { senha: 'NovaSenha123' });
    ...
    it('deve lançar erro ao tentar atualizar usuário inexistente', async () => {
      const idFake = new mongoose.Types.ObjectId();
      await expect(usuarioRepository.atualizar(idFake, { nome: 'X' })).rejects.toThrow(CustomError);
    });
  });
  ...
  describe('Relacionamento com cursos e progresso', () => {
    ...
    it('deve registrar progresso em cursos', async () => {
      ...
      const usuario = await usuarioRepository.criar({
        ...
        senha: await bcrypt.hash('Senha@123', 10),
        progresso: [{ curso: curso1, percentual_conclusao: '80' }],
      });
    ...
    it('deve popular cursosIds e progresso.curso ao buscar usuário', async () => {
      ...
      const usuario = await usuarioRepository.criar({
        ...
        progresso: [{ curso: curso1._id, percentual_conclusao: 100 }],
      });
      const req = { params: { id: usuario._id.toString() }, query: {} };
      const resultado = await usuarioRepository.listar(req);
  ...
  describe('Cobertura de branches e erros raros', () => {
    it('deve lançar erro se filterBuilder.build não for função', async () => {
      ...
      // Mock do método build para não ser uma função
      UsuarioFilterBuilder.prototype.build = 'não é uma função';

      const repo = new UsuarioRepository();
      const req = { params: {}, query: {} }; // params vazio para não entrar no fluxo de busca por ID
  ...
  describe('Busca por email', () => {
    ...
    it('deve buscar usuário por email ignorando um ID específico', async () => {
      const usuarioExistente = await UsuarioModel.findOne({ email: 'teste@email.com' });
  ...
  describe('Ordenação e filtros especiais', () => {
    beforeEach(async () => {
      await UsuarioModel.create([
        { nome: 'Alpha', email: 'alpha@teste.com', senha: await bcrypt.hash('Senha@123', 10), ativo: true },
        { nome: 'Beta', email: 'beta@teste.com', senha: await bcrypt.hash('Senha@123', 10), ativo: true },
        { nome: 'Charlie', email: 'charlie@teste.com', senha: await bcrypt.hash('Senha@123', 10), ativo: true },
      ]);
    });
  ...
  describe('Métodos especializados', () => {
      ...
      await UsuarioModel.create([
        {
          ...
          senha: await bcrypt.hash('Senha@123', 10),
          cursosIds: [cursoId],
          progresso: [{ curso: cursoId, percentual_conclusao: '50' }]
        },
        {
          nome: 'Usuario2',
          ...
          senha: await bcrypt.hash('Senha@123', 10),
          cursosIds: [cursoId],
          progresso: [{ curso: cursoId, percentual_conclusao: '20' }]
        },
        {
          nome: 'Usuario3',
          ...
          senha: await bcrypt.hash('Senha@123', 10),
          cursosIds: [cursoId],
          progresso: [{ curso: cursoId, percentual_conclusao: '80' }]
        }
      ]);
    });
    ...
    it('deve buscar usuários com progresso significativo usando limite padrão', async () => {
      ...
      expect(usuarios.length).toBe(2); // usuarios com 50% e 80% (>= 30% padrão)
    });
    ...
    it('deve remover referências de curso dos usuários', async () => {
      ...
      // Verifica se as referências foram realmente removidas
      const usuarios = await UsuarioModel.find({ cursosIds: cursoId });
      expect(usuarios.length).toBe(0);
      
      const usuariosComProgresso = await UsuarioModel.find({ 'progresso.curso': cursoId });
  ...
  describe('Enriquecimento de dados do usuário', () => {
    it('deve enriquecer usuário com estatísticas de progresso', async () => {
      ...
      // Criar cursos reais para o populate funcionar
      await CursoModel.create([
        { _id: cursoId1, titulo: 'Curso 1', cargaHorariaTotal: 10, status: 'ativo', criadoPorId: new mongoose.Types.ObjectId() },
        { _id: cursoId2, titulo: 'Curso 2', cargaHorariaTotal: 20, status: 'ativo', criadoPorId: new mongoose.Types.ObjectId() },
        { _id: cursoId3, titulo: 'Curso 3', cargaHorariaTotal: 30, status: 'ativo', criadoPorId: new mongoose.Types.ObjectId() }
      ]);
      ...
      const usuarioCriado = await UsuarioModel.create({
        ...
        progresso: [
          { curso: cursoId1, percentual_conclusao: '100' }, // concluído
          { curso: cursoId2, percentual_conclusao: '50' }   // em andamento
          // cursoId3 não tem progresso
        ]
      });

      const req = { params: { id: usuarioCriado._id.toString() }, query: {} };
      const resultado = await usuarioRepository.listar(req);
    ...
    it('deve enriquecer usuário sem progresso', async () => {
      ...
      // Criar curso real para o populate funcionar
      await CursoModel.create({
        _id: cursoId, 
    ...
    it('deve enriquecer usuário com progresso zero', async () => {
      ...
      const usuarioCriado = await UsuarioModel.create({
        ...
        cursosIds: [cursoId],
        progresso: [
          { curso: cursoId, percentual_conclusao: '0' }
        ]
      });
